...
use bitcoin::{
    opcodes, script, OutPoint, Script, ScriptBuf, Sequence, Transaction, TxIn, TxOut, Witness,
};

use crate::utils::{vec_to_u81, vec_to_u82, vec_to_u820};
...
impl CoreDao {
    ...
    pub fn construct(&mut self) -> (TxOut, ScriptBuf) {
        ...
        (op_return.clone(), new_script_buf.clone())
    }

    pub fn decode_tx(transaction: &Transaction) -> Result<(CoreOption, Vec<u8>), String> {
        ...
        let (lock_time, staker) = Self::from_script(Script::from_bytes(rest.as_slice()))
    ...
    fn from_script(script: &Script) -> ScriptResult<(Option<u32>, Option<[u8; 20]>)> {
...
pub fn create_new_option_script() {
    let option = CoreOption {
        ...
        validator: vec![0; 20],
        fee: 1,
        pub_key: "02afee55a2cdcb6c47a593d629b04e13399354d348a3d84ad19310e2b6396e7237".to_string(),
        lock_time: 848484,
        network: bitcoin::Network::Bitcoin,
    };
    let mut core_dao = CoreDao::new(option.clone());
    let (op_return, script) = core_dao.construct();
    ...
    let (_option, staker) = CoreDao::decode_tx(&tx).unwrap();
